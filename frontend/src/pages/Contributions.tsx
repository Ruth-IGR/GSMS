...
import { fetchContributionLimitByGoal } from "../api/contributionLimits";
...
const Contributions = () => {
  ...
  // Fetch contribution limit for selected goal
  const { data: contributionLimit } = useQuery({
    queryKey: ["contribution-limit", form.goalId],
    queryFn: () => fetchContributionLimitByGoal(Number(form.goalId)),
    enabled: isAuthed && !!form.goalId,
    retry: (failureCount, error: any) => {
        ...
        <div className="grid gap-5 sm:grid-cols-3">
          ...
          <div>
            ...
            <input
              type="number"
              required
              min={contributionLimit?.fixedAmount || contributionLimit?.minimumAmount || 0}
              max={contributionLimit?.maximumAmount || undefined}
              step="0.01"
              value={form.amount}
              onChange={(e) =>
              ...
              className="input"
              placeholder={contributionLimit?.fixedAmount 
                ? `Minimum: ${contributionLimit.fixedAmount.toFixed(2)} (can contribute more)`
                : contributionLimit?.minimumAmount
                ? `Minimum: ${contributionLimit.minimumAmount.toFixed(2)}`
                : "0.00"}
            />
            {contributionLimit && (
              <div className="mt-2 space-y-1 rounded-md bg-blue-50 p-3 text-xs border border-blue-200">
                {contributionLimit.fixedAmount && (
                  <div>
                    <p className="font-semibold text-blue-800">
                      Fixed Amount (Minimum Required): ${contributionLimit.fixedAmount.toFixed(2)}
                    </p>
                    <p className="mt-1 text-blue-600 italic">
                      💡 You can contribute more than this amount if you wish. The full amount will count toward the goal.
                    </p>
                  </div>
                )}
                {!contributionLimit.fixedAmount && contributionLimit.minimumAmount && (
                  <p className="text-blue-700">
                    Minimum: ${contributionLimit.minimumAmount.toFixed(2)}
                  </p>
                )}
                {contributionLimit.maximumAmount && (
                  <p className="text-blue-700">
                    Maximum per contribution: ${contributionLimit.maximumAmount.toFixed(2)}
                  </p>
                )}
                {contributionLimit.maximumTotalPerUser && (
                  <p className="text-blue-700">
                    Maximum total per user for this goal: ${contributionLimit.maximumTotalPerUser.toFixed(2)}
                  </p>
                )}
              </div>
            )}
